...
def compute_embeddings(prompt, tokenizer, model, device, original_prompt_len=None):
    with t.no_grad():
        # Tokenize the entire prompt for the main model
...
def compute_embedding_trajectories(prompts, tokenizer, model, device, max_tokens_gen=500, generation_kwargs={}):
    ...
    for prompt in prompts:
        ...
        for i in range(max_tokens_gen):
            ...
            # inputs = tokenizer(generated_text, return_tensors='pt', padding=True).to(device)
...
plt.legend()
plt.show()
plt.savefig("/images/embedding_activations", dpi=300, bbox_inches="tight")
